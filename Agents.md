...
**结论：** 节点标签中可以用 `\n`，但绝不能出现 `"`（ASCII 直引号）或 `"` `"`（Unicode 弯引号）。

**⚠️ 修复陷阱：** 修复某个 Mermaid 语法错误（如补全括号）后，**必须同步扫描同一图表的所有节点标签，确认没有双引号残留**。单独修括号而忽略引号，图表仍会报错。

```bash
# 快速检查文件内所有 Mermaid 节点是否含引号（在 wiki-clone 目录下执行）
grep -rn '\"' content/ | grep -v '^Binary'
```
